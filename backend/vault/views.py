...
class AuditEventViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        # "organization" is serialized as PK, so join is not necessary and removed to reduce database overhead.
...
class AccessGrantViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        # Bolt Optimization: use select_related('grantee') for serialization.
        # Redundant select_related joins for 'vault_item' and 'granted_by' are removed as they are serialized as PrimaryKey IDs.
        return (
